...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$productPictures = CMarsShopHelper::getDoublePicturesForItem($arItem, $arParams['ADD_PICT_PROP'], $resizeData);
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$offerPictures = CMarsShopHelper::getDoublePicturesForItem($arOffer, $arParams['OFFER_ADD_PICT_PROP'], $resizeData);
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$arOneRow = array(
						...
						'PRICE' => (isset($arOffer['ITEM_PRICES'][0]['RATIO_PRICE']) ? $arOffer['ITEM_PRICES'][0]['RATIO_PRICE'] : $arOffer['ITEM_PRICES'][0]['MIN_PRICE']),
						'BASIS_PRICE' => (isset($arOffer['ITEM_PRICES'][0]['RATIO_PRICE']) ? $arOffer['ITEM_PRICES'][0]['RATIO_PRICE'] : $arOffer['ITEM_PRICES'][0]['MIN_PRICE']),
	...
	$db_enum_list = CIBlockProperty::GetPropertyEnum($arParams['LABEL_PROP'], Array("SORT" => "ASC"), Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"]));
	while($ar_enum_list = $db_enum_list->GetNext()) {
		$arResult["UL_TABS"][] = $ar_enum_list;
	}
