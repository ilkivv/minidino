...
window.JCCatalogTopSection.prototype.UpdateRow = function(intNumber, activeID, showID, canBuyID)
{
	...
	if (-1 < intNumber && intNumber < this.obTreeRows.length)
	{
		...
		if (!!RowItems && 0 < RowItems.length)
		{
			...
			strNewLen = (extShowMode ? (100/countShow)+'%' : '15%');
			obData = {
				props: { className: '' },
				style: {
					width: strNewLen
...
window.JCCatalogTopSection.prototype.setPrice = function(price)
{
	...
	if (!!this.obPrice)
	{
		strPrice = '<ins>' + price + ' руб</ins>';
		if (this.showOldPrice && (price.DISCOUNT_VALUE !== price.VALUE))
		{
			strPrice += ' <del>'+BX.Currency.currencyFormat(price.VALUE, price.CURRENCY, true)+'</del>';
		}
	...
	else
	{
		...
		if (successful) {
			...
			if (this.showClosePopup) {
				strClosePopup = '<a href="javascript:$.magnificPopup.close();" class="rightmargin-sm">'+ BX.message("BTN_MESSAGE_CLOSE_POPUP") + '</a>';
			}
			$.magnificPopup.open({
			  items: {
			    ...
			    + '<img src="'+strPict+'" style="max-height:400px">'
			    + '</div>'
			...
			$("#header-notify").attr("data-notify-msg",BX.message('BASKET_UNKNOWN_ERROR'));
			$("#header-notify").attr("data-notify-type", "error");
		}
		SEMICOLON.widget.notifications("#header-notify");
	}
