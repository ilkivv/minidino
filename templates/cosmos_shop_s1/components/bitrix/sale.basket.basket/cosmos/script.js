        ...
        for (i = 0; i < arColumns.length; i++)
        {
            switch (arColumns[i])
            {
                ...
                case 'NAME':
                    ...
                    oCellName.setAttribute('class', 'cart-product-thumbnail');
                    ...
                    cellItemHTML = '';
                    oCellItem.setAttribute('class', 'cart-product-name');
                ...
                case 'QUANTITY':
                    ...
                    oCellQuantity.setAttribute('class', 'cart-product-quantity');
                ...
                case 'PRICE':
                    ...
                    oCellPrice.setAttribute('class', 'cart-product-price');
    ...
    if (!!res.BASKET_DATA)
    {
        for (id in res.BASKET_DATA.GRID.ROWS)
        {
            if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
            {
                ...
                if (item.DISCOUNT_PRICE_PERCENT_FORMATED == "0%" || item.DISCOUNT_PRICE_PERCENT_FORMATED == "") {
                    $('#discount_info_' + id).hide(500);
                } else {
                    $('#discount_info_' + id).show(500);
                }
                if (BX('discount_value_' + id))
                    BX('discount_value_' + id).innerHTML = '-' + item.DISCOUNT_PRICE_PERCENT_FORMATED;

                if (BX('discount_value_num_' + id)) {
                    var newDiscPrice = BX.Currency.currencyFormat(item.DISCOUNT_PRICE, item.CURRENCY, true);
                    BX('discount_value_num_' + id).innerHTML = newDiscPrice;
                }
...
$(document).ready(function() {
    loadFlyCart();
    $('.fly-cart .fly-cart-trigger').click(function () {
        $('.fly-cart').toggleClass('fly-cart-active', 300);
        loadFlyCart();
    });

    $('#content, .fly-cart').on("click", ".fly-cart-btn", function(){
        SEMICOLON.widget.showFormProcess($("#basket_form_container"));
        $.ajax({
            url: $(this).attr('href'),
            complete: function(){
                if ($( ".fly-cart-body" ).length > 0) {
                    loadFlyCart();
                } else {
                    $("#basket_form").load(boxsolCosmos.cosmosSiteDir + 'personal/cart/?cosmos_ajax=Y #basket_form', function(){
                        SEMICOLON.widget.tabs();
                        SEMICOLON.widget.hideFormProcess($("#basket_form_container").parent());
                    });
                }
                return false;
            }
    ...
    BX.addCustomEvent(window, 'OnBasketChange', loadFlyCart);
});


function loadFlyCart() {
    $( ".fly-cart-body" ).load( boxsolCosmos.cosmosSiteDir + "personal/cart/?cosmos_ajax=Y", function(){
        $('.fly-cart .ajax-modal-title').hide();
        $('.fly-cart .modal-padding').removeClass("modal-padding");
        $('.fly-cart .bx_ordercart_coupon').hide();

        $('.fly-cart #basket_items, .fly-cart #delayed_items').css({
            "max-height": "260px",
            "display"   : "block",
            "overflow"  : "auto",
            "overflow-x": "none !important"
        });
        $("#basket_items_delayed").removeClass("bottommargin");
        $('.fly-cart .cart-product-name').css("max-width", "240px");
        $('.fly-cart .cart-product-price').css("max-width", "120px");
        $('.fly-cart .tabs, .fly-cart form').css("margin-bottom", "0px");

        if ($('.fly-cart #basket_items .cart-product-name').length > 0) {
            $('.fly-cart .fly-cart-trigger span').html($('.fly-cart #basket_items .cart-product-name').length - 1);
        } else {
            $('.fly-cart .fly-cart-trigger span').html(0);
        }
        SEMICOLON.widget.tabs();
        $('.fly-cart').removeClass('hidden');
    });
}