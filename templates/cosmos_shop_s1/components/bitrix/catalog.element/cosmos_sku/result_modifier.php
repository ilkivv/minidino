...
$arDefaultParams = array(
	...
	'BRAND_USE' => 'N',
	'BRAND_PROP_CODE' => '',
	'DETAIL_USE_IMAGE_RESIZE' => "Y",
    'DETAIL_IMAGE_RESIZE_WIDTH' => 670,
    'DETAIL_IMAGE_RESIZE_HEIGHT' => 800,
	'DETAIL_IMAGE_RESIZE_TYPE' => BX_RESIZE_IMAGE_PROPORTIONAL,
);
$arParams = array_merge($arDefaultParams, $arParams);


$resizeData = false;
if ($arParams["DETAIL_USE_IMAGE_RESIZE"] == "Y") {
    $resizeData = array(
        "width" => intval($arParams['DETAIL_IMAGE_RESIZE_WIDTH']),
        "height" => intval($arParams['DETAIL_IMAGE_RESIZE_HEIGHT']),
        "resize_type" => intval($arParams['DETAIL_IMAGE_RESIZE_TYPE']),
    );
}
$resizeDataThumb = array(
    "width" => 75,
    "height" => 75,
...
if (is_array($resizeData) && $arParams["DETAIL_USE_IMAGE_RESIZE"] == "Y") {
    foreach ($productSlider as $keyPicture => $picture) {
        ...
        $productSlider[$keyPicture]["ORIGINAL_SRC"] = $picture['SRC'];
        $productSlider[$keyPicture]["SRC"] = $arFileTmp["src"];
        $productSlider[$keyPicture]["WIDTH"] = $arFileTmp["width"];
        $productSlider[$keyPicture]["HEIGHT"] = $arFileTmp["height"];
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		if (is_array($resizeData) && $arParams["DETAIL_USE_IMAGE_RESIZE"] == "Y") {
		    foreach ($offerSlider as $keyPicture => $picture) {
		        $pictureId = intval($picture['ID']);
		        if ($pictureId == 0) return;
		        $arFileTmp = CFile::ResizeImageGet(
		            $pictureId,
		        ...
		        $offerSlider[$keyPicture]["ORIGINAL_SRC"] = $picture["SRC"];
		        $offerSlider[$keyPicture]["SRC"] = $arFileTmp["src"];
		        $offerSlider[$keyPicture]["WIDTH"] = $arFileTmp["width"];
		        $offerSlider[$keyPicture]["HEIGHT"] = $arFileTmp["height"];
		        ...
		        $offerSlider[$keyPicture]["THUMB_SRC"] = $arFileTmpThumb["src"];
