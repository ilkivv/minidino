...
$hiddenIconList = "hideparam-grid";
$hiddenPreviewText = "hideparam-grid";
if ($arParams["SECTION_HIDE_ICONLIST"] == "N") {
	$hiddenIconList = "";
}
if ($arParams["SECTION_HIDE_PREVIEW_TEXT"] == "N") {
	$hiddenPreviewText = "";
}

$arParams['USE_PRODUCT_QUANTITY'] = "N"; // may be work later, need design
?><!-- --- -->
<?
if (!empty($arResult['ITEMS']))
{
	$templateLibrary = array('popup', 'ajax');
	$currencyList = '';
...
<!-- Shop
============================================= -->
<div id="shop" class="shop grid-container product-<?=$arParams['LINE_ELEMENT_COUNT']; ?><?=($arParams["TABLE_VIEW"] == "Y")? " products-table" : "" ?> clearfix bx_catalog_list_home <?=($arParams["LINE_ELEMENT_COUNT"] == 1)? "noautomaxheight" : "" ?>" data-layout="fitRows">
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?>
	<div id="<? echo $strMainID; ?>" class="product clearfix">

		<div class="product-image">
			<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $imgTitle; ?>">
				<img id="<? echo $arItemIDs['PICT']; ?>" src="<?=$arItem['DETAIL_PICTURE']["SRC"]?>" alt="<?=$imgAlt?>">
				<?
					if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE'])
				{
					?> <img class="product-image__discount-icon" src="/images/percent.svg" width="61" height="74"><?
				}
				?>
			</a><?
			if ($arItem['PREVIEW_PICTURE_SECOND']['ID'])
			{
				?><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $imgTitle; ?>">
				<img id="<? echo $arItemIDs['SECOND_PICT']; ?>" src="<?=(($arItem['PREVIEW_PICTURE_SECOND']["ID"]>0)?$arItem['PREVIEW_PICTURE_SECOND']["SRC"]:$arItem['DETAIL_PICTURE']["SRC"])?>" alt="<?=$imgAlt?>">
				<?
					if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE'])
				{
					?> <img class="product-image__discount-icon" src="/images/percent.svg" width="61" height="74"><?
			...
			if ($arItem['LABEL'])
			{
				...
				?>
				<div id="<? echo $arItemIDs['STICKER_ID']; ?>" class="sale-flash" title="<? echo $arItem['LABEL_VALUE']; ?>"><? echo $arItem['LABEL_VALUE']; ?></div>
				<?
			}
			if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT'])
			{
				?>
				<div id="<? echo $arItemIDs['DSC_PERC']; ?>" class="sale-flash" style="<?=$styleTopForDiscount?>display:<? echo (0 < $minPrice['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;">-<? echo $minPrice['DISCOUNT_DIFF_PERCENT']; ?>%</div>
				<?
			}
			unset($styleTopForDiscount);
			?>

			<div class="product-overlay hidden-on-load">
				...
				$arJSParams = array(
					...
					'SECOND_PICT' => $arItem['DETAIL_PICTURE'],
				...
				$arJSParams = array(
					...
					'DEFAULT_PICTURE' => array(
						'PICTURE' => $arItem['DETAIL_PICTURE'],
						'PICTURE_SECOND' => $arItem['DETAIL_PICTURE']
					),
					'VISUAL' => array(
						'ID' => $arItemIDs['ID'],
						'PICT_ID' => $arItemIDs['PICT'],
				...
				if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
				{
					...
					if ($arItem['CAN_BUY'])
					{
						if ('Y' == $arParams['USE_PRODUCT_QUANTITY']) // may be work later
						{
							?>
						...
						if ($arParams['DISPLAY_COMPARE'])
						{
							...
							<div class="bx_catalog_item_controls_blocktwo">
								<a id="<? echo $arItemIDs['COMPARE_LINK']; ?>" href="javascript:void(0)" class="item-quick-view">
									<i class="icon-line-bar-graph-2"></i><span> <? echo $compareBtnMessage; ?></span></a>
							</div><?
						} else {
							?>
				...
				?>
					<div class="">
						...
						<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">
							<a id="<? echo $arItemIDs['BUY_LINK']; ?>" href="javascript:void(0)" class="add-to-cart" rel="nofollow">
								<i class="icon-shopping-cart"></i><span><?
								if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
								{
									echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
	...
	<div class="product-desc">
		...
		<div class="product-title"><h3><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a></h3></div>
		<div class="product-price"><div id="<? echo $arItemIDs['PRICE']; ?>"><?
		if (!empty($minPrice))
		{
			if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE'])
			{
				?> <del><? echo $minPrice['PRINT_VALUE']; ?></del><?
			}
			echo "<ins>";
		...
		{
			?>
			<ul class="iconlist <?=$hiddenIconList?>">
				<?
...
<div class="catalog-pagination__wrapper col_full topmargin">
<?
...
<?
?>
<? if ($arResult["NAV_RESULT"]->NavPageNomer == 1&& strlen($arResult["DESCRIPTION"]) > 0) :?>
