...
//			$locationChanged = $locValue != $this->request->get('RECENT_DELIVERY_VALUE');
	...
	protected function obtainTotal()
	{
        ...
        /*
         * бесплатная доставка от 4к
         * */
		if ($_SERVER['REMOTE_ADDR'] == '5.77.5.203' || true) {
            $delivery = $userResult['DELIVERY_ID'];
            foreach ($arResult['DELIVERY'] as $deliv) {
                if ($deliv['CHECKED'] == 'Y') {
                    $newPrice = $deliv['PRICE'];
                }
            }
		    ...
		    foreach ($this->order->getShipmentCollection() as &$shipment)
		    {
                if (!$shipment->isSystem()) {
                    $shipment->setField('PRICE_DELIVERY',$newPrice);
                    $shipment->setField('BASE_PRICE_DELIVERY',$newPrice);
	...
	protected function initPersonType(Order $order)
	{
		...
		foreach ($personTypes as $personType)
		{
		    ...
		    if ((in_array(8, CUser::GetUserGroup($USER->GetID())) && $personType["ID"]  == 2) || (!in_array(8, CUser::GetUserGroup($USER->GetID())) && $personType["ID"]  == 1) )
			{
		...
		/*
        if ($_SERVER['REMOTE_ADDR'] == '5.77.5.203') {
    		print_r($arResult["PERSON_TYPE"]);
    		print_r(CUser::GetUserGroup($USER->GetID()));
        }*/
		if ($personTypeId == 0)
	...
	protected function saveOrder($saveToSession = false)
	{
		...
		$this->initAffiliate();
		/*
		 * бесплатная доставка для суммы от 4к
		 * */
        if ($_SERVER['REMOTE_ADDR'] == '5.77.5.203' || true){
            $this->obtainBasket();
            
            $this->calculateDeliveries($this->order);
            foreach ($arResult['BASKET_ITEMS'] as $it) {
                $total +=$it['PRICE']*$it['QUANTITY'];
            }
            
            if ($total >= 4000) {
                $arDelivery = $arResult['DELIVERY'];
                $prices  = array_column($arResult['DELIVERY'], 'PRICE');
                array_multisort($prices, SORT_ASC, $arResult['DELIVERY']);
    		...
    		foreach ($this->order->getShipmentCollection() as &$shipment)
    		{
    		    if (!$shipment->isSystem()) {
    		        ...
    		        $shipment->setField('BASE_PRICE_DELIVERY',$newPrice);
    		        $shipment->setField('CUSTOM_PRICE_DELIVERY','Y');
    		        $this->recalculatePayment($this->order);
    		    }
    		}
	   }
		$res = $this->order->save();
